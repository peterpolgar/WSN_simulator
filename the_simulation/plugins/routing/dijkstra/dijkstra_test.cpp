...
int Where_to_send(unsigned source_idx, std::vector<std::vector<int>>* metric_matrix)
{
    long long best_koltseg = 123987654321;
    int best_dest_id = -1;
    unsigned celpontok[3]{ 3, 4, 5 };
    unsigned number_of_sinks = 3;
    for ( unsigned z = 0; z < number_of_sinks; ++z ) {
        unsigned i = celpontok[z];
        ...
        while ( Q.size() > 0 ) {
            ...
            for ( int v = 0; v < (*metric_matrix)[u].size(); ++v ) {
